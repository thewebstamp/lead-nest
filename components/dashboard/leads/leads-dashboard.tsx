...
const statusOptions = [
    ...
    { value: "booked", label: "Booked", color: "bg-green-500/20 text-green-400 border-green-500/30" },
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-300">Manage and track your incoming leads</p>
            </div>

            {/* Stats Cards - Dark glass style */}
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <Card className="border border-white/10 bg-gray-800/30 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all">
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-300">Contacted</p>
                                <p className="text-2xl font-bold text-amber-400 mt-1">{stats.contacted}</p>
                            </div>
                ...
                <Card className="border border-white/10 bg-gray-800/30 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all">
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-300">Quoted</p>
                                <p className="text-2xl font-bold text-purple-400 mt-1">{stats.quoted}</p>
                            </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-green-400 mt-1">{stats.booked}</p>
                            </div>
                ...
                <Card className="border border-white/10 bg-gray-800/30 backdrop-blur-xl shadow-lg hover:shadow-xl transition-all">
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-300">Lost</p>
                                <p className="text-2xl font-bold text-red-400 mt-1">{stats.lost}</p>
                            </div>
                ...
                <div className="flex-1">
                    <div className="relative">
                        ...
                        <Input
                            ...
                            className="pl-10 border-gray-700 bg-gray-800/50 text-white placeholder-gray-500 focus:border-blue-500 focus:ring-blue-500 rounded-xl"
                            value={searchTerm}
            ...
            {/* Bulk Actions - Dark glass panel */}
            {selectedLeads.size > 0 && (
                <div className="p-4 bg-blue-500/10 backdrop-blur-sm border border-blue-500/20 rounded-xl flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Select onValueChange={handleBulkStatusUpdate}>
                            <SelectTrigger className="w-50 border-blue-500/20 bg-blue-500/10 text-blue-400 focus:border-blue-500 focus:ring-blue-500 rounded-xl">
            ...
            {/* Leads Table - Dark glass card */}
            <Card className="border border-white/10 bg-gray-800/30 backdrop-blur-xl shadow-lg overflow-hidden">
                <div className="overflow-x-auto">
                    <Table>
                        <TableHeader className="bg-gray-800/50 border-b border-white/10">
                            <TableRow>
                                <TableHead className="w-12">
                                    <input
                                        type="checkbox"
                                        className="h-4 w-4 rounded border-gray-600 bg-gray-800/50 text-blue-600 focus:ring-blue-500/50 focus:ring-offset-gray-900"
                                        onChange={(e) => {
                                            if (e.target.checked) {
                                ...
                                <TableHead className="font-semibold text-gray-200">Lead</TableHead>
                                <TableHead className="font-semibold text-gray-200">Service</TableHead>
                                <TableHead className="font-semibold text-gray-200">Location</TableHead>
                                <TableHead className="font-semibold text-gray-200">Status</TableHead>
                                <TableHead className="font-semibold text-gray-200">Priority</TableHead>
                                <TableHead className="font-semibold text-gray-200">Received</TableHead>
                                <TableHead className="text-right font-semibold text-gray-200">Actions</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredLeads.length === 0 ? (
                                    ...
                                    <TableCell colSpan={8} className="text-center py-12">
                                        {leads.length === 0 ? (
                                            <div className="text-center">
                                                ...
                                                <p className="text-sm text-gray-300">Share your form link to start receiving leads</p>
                                            </div>
                                        ) : (
                                            <div className="text-center">
                                                ...
                                                <p className="text-lg font-semibold text-white mb-2">No matching leads</p>
                                ...
                                filteredLeads.map((lead) => (
                                    <TableRow
                                        key={lead.id}
                                        className="hover:bg-gray-700/30 transition-colors cursor-pointer border-white/10"
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            <div className="font-medium text-white">{lead.service_type}</div>
                                            {lead.message && (
                                                <div className="text-sm text-gray-300 truncate max-w-50">
                                                    {lead.message.substring(0, 50)}
                                                    {lead.message.length > 50 ? "..." : ""}
                                                </div>
                                            )}
                                        </TableCell>
                                        <TableCell className="text-gray-200">{lead.location}</TableCell>
                                        <TableCell onClick={(e) => e.stopPropagation()}>
                                            <Select
                                                value={lead.status}
                                        ...
                                        <TableCell onClick={(e) => e.stopPropagation()}>
                                            <Badge
                                                className={cn(
                                                    ...
                                                    priorityColors[lead.priority] || "bg-gray-500/20 text-gray-300 border-gray-500/30"
                                                )}
                                            >
                                        ...
                                        <TableCell>
                                            <div className="text-sm text-gray-200">{formatDate(lead.created_at)}</div>
                                            <div className="text-xs text-gray-400">{getTimeAgo(lead.created_at)}</div>
                                        </TableCell>
                                            ...
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        className="h-8 w-8 p-0 rounded-lg hover:bg-gray-700/50"
                                                    >
            ...
            <Card className="border border-white/10 bg-gray-800/30 backdrop-blur-xl shadow-lg">
                <CardHeader>
                    <CardTitle className="text-xl font-semibold text-white">Lead Pipeline</CardTitle>
                    <CardDescription className="text-gray-300">
                        Visual representation of your leads through the pipeline
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="flex items-center justify-between h-8 bg-gray-700 rounded-full overflow-hidden">
                        {[
                    ...
                    <div className="flex flex-wrap justify-between mt-3 text-sm text-gray-300">
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-blue-500" /> New: {stats.new}
                        </span>
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-amber-500" /> Contacted: {stats.contacted}
                        </span>
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-purple-500" /> Quoted: {stats.quoted}
                        </span>
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-green-500" /> Booked: {stats.booked}
                        </span>
                        <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-red-500" /> Lost: {stats.lost}
                        </span>
                    </div>
                </CardContent>
            </Card>
